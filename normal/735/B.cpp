...
int main()
{
   ...
   // for(auto i:city2)
     // cout<<i<<sp;
      ...
      double sum=0,sum1=0;
      sum=accumulate(all(city1),sum);
      sum1=accumulate(all(city2),sum1);
      //deb(sum);
      //deb(sum1);
      double ans=double(sum/city1.size());
      double ans1=double(sum1/city2.size());
      //deb(ans);
     // deb(ans1);
      cout<<(double)(ans+ans1);
	return 0;
}