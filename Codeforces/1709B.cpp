  ...
  */
  ...
  int32_t main()
  {
    ...
    ll n,q;
    cin>>n>>q;
    vll ara(n),prefix_sum(n),suffix_sum(n);
    for(int i=0;i<n;i++)
    cin>>ara[i];

    for(int i=1;i<n;i++)
    prefix_sum[i]=prefix_sum[i-1]+max(0LL,ara[i-1]-ara[i]);

    for(int i=n-2;i>=0;i--)
    suffix_sum[i]=suffix_sum[i+1]+max(0LL,ara[i+1]-ara[i]);
    debug(prefix_sum);
    debug(suffix_sum);
    while(q--)
    {
        ...
        l--,r--;
        if(l<=r)
        cout<<prefix_sum[r]-prefix_sum[l]<<nn;
        else 
        cout<<suffix_sum[r]-suffix_sum[l]<<nn;
    }
